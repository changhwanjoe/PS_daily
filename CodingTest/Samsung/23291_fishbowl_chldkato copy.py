...
sys.stdin = open("23291.txt","r")

N, K = map(int, sys.stdin.readline().split()) # 8 1
direction = [(-1, 0), (1, 0), (0, -1), (0, 1)] 
board = list()
board.append(deque(list(map(int, sys.stdin.readline().split())))) # 5 2 3 14 9 2 11 8
print(board[0]) # board = deque([5, 2, 3, 14, 9, 2, 11, 8])
#sys.exit()
...
answer = 0
while True: # N,K = 8 1
    result = get_result(board) # board = deque([5, 2, 3, 14, 9, 2, 11, 8])
    if result <= K:
        print(answer)
        break
